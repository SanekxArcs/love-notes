...
export default async function JoinOpenGraphImage({ searchParams }: { searchParams?: Promise<{ title?: string; message?: string }> }) {
  const { title, message } = await (searchParams ?? Promise.resolve<{ title?: string; message?: string }>({}));
  const cleanTitle = cleanText(title, "Створіть ваш простір для двох", 90);
  const cleanMessage = cleanText(message, "Теплі слова, спільні плани та важливі спогади.", 130);
  ...
  return new ImageResponse(
    <div style={{ height: "100%", width: "100%", display: "flex", position: "relative", overflow: "hidden", background: "linear-gradient(135deg, #fff4fa 0%, #f9d5e8 46%, #d8c7fa 100%)", color: "#351726", fontFamily: "sans-serif" }}>
      ...
      <div style={{ zIndex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "72px 64px", width: 760 }}>
        ...
        <div style={{ display: "flex", marginTop: 40, fontSize: 65, lineHeight: 1.05, fontWeight: 800, letterSpacing: -2 }}>{cleanTitle}</div>
        <div style={{ display: "flex", marginTop: 24, width: 590, fontSize: 29, lineHeight: 1.35, color: "#6a4b5d" }}>{cleanMessage}</div>
